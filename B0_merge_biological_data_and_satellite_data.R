...
dataset <- read.csv(filename_dataset, header=TRUE, sep=";", dec=".", colClasses= 'character')
...
dataset <- subset(dataset, !is.na(Year) & !is.na(Month) & !is.na(Day))
...
if (length(file_satellite_parameter)>0) { 
  ...
  #merging process
  dataset_modis_chl <- merge(dataset,satellite_parameters_b, all.x = TRUE)
  dataset_modis_chl$year <- NULL
  dataset_modis_chl$month <- NULL
  dataset_modis_chl$day <- NULL
  dataset_modis_chl$longitude <- NULL
  dataset_modis_chl$latitude <- NULL
  #deleting duplicated rows
  dataset_modis_chl_b <- unique(dataset_modis_chl)
}
...
if (length(file_satellite_parameter)>0) {
  ...
  #merging process
  dataset_modis_sst <- merge(dataset,satellite_parameters_b, all.x = TRUE)
  dataset_modis_sst$year <- NULL
  dataset_modis_sst$month <- NULL
  dataset_modis_sst$day <- NULL
  dataset_modis_sst$longitude <- NULL
  dataset_modis_sst$latitude <- NULL
  #deleting duplicated rows
...
if (length(file_satellite_parameter)>0) {
  ...
  #merging process
  dataset_avhrr <- merge(dataset,satellite_parameters_b, all.x = TRUE)
  dataset_avhrr$year <- NULL
  dataset_avhrr$month <- NULL
  dataset_avhrr$day <- NULL
  dataset_avhrr$longitude <- NULL
  dataset_avhrr$latitude <- NULL
  #deleting duplicated rows
...
if (dataset_id == "6"){
  ...
  dataset_modis_chl_b <- orderBy(~ Key, dataset_modis_chl_b)
  dataset_modis_sst_b <- orderBy(~ Key, dataset_modis_sst_b)
  dataset_satellite <- cbind(dataset,dataset_modis_chl_b$bathy, dataset_modis_chl_b$modis.chla.avg_r0, dataset_modis_chl_b$modis.chla.avg_r1, dataset_modis_chl_b$modis.chla.avg_r2, dataset_modis_chl_b$modis.chla.min_r2, dataset_modis_chl_b$modis.chla.max_r2, dataset_modis_chl_b$modis.chla.std_r2, dataset_modis_chl_b$modis.chla.mode_r2,
                             dataset_modis_sst_b$modis.sst.avg_r0, dataset_modis_sst_b$modis.sst.avg_r1, dataset_modis_sst_b$modis.sst.avg_r2, dataset_modis_sst_b$modis.sst.min_r2, dataset_modis_sst_b$modis.sst.max_r2, dataset_modis_sst_b$modis.sst.std_r2, dataset_modis_sst_b$modis.sst.mode_r2)
  colnames(dataset_satellite) =  c(colnames(dataset),"Bathymetry","modis.chla.avg_r0","modis.chla.avg_r1","modis.chla.avg_r2","modis.chla.min_r2","modis.chla.max_r2","modis.chla.std_r2","modis.chla.mode_r2",
                                   "modis.sst.avg_r0","modis.sst.avg_r1","modis.sst.avg_r2","modis.sst.min_r2","modis.sst.max_r2","modis.sst.std_r2","modis.sst.mode_r2")
}
