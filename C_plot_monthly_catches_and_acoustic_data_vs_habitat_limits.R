...
species_presence[sst<9 | sst>26] <- 0
species_presence[sst>=9 & sst<=26] <- 1
species_presence[chla<0.07] <- 0
plot(CHLA,SST,xlab = "CHLA (mg/m3)", ylab = "SST (°C)", xlim=c(0,2),ylim=c(9,30), 
...
legend("top", c("Netherlands catches 2005-2011",                 
                ...
                lwd = 2, col = c("green","red","blue","darkgreen"))
...
for (ind_month in 1:12) {
  ...
  image(lon_pacific,lat_pacific,species_presence_pacific, col=rev(brewer.pal(9,"RdBu")), xlab='Longitude', ylab ='Latitude', 
        main= paste("Horizontal habitat limits - Month ", ind_month,
